...
(LOOP)                  
    ...
    D=M
    @WHITE
    D;JEQ  //if no key is pressed do the white loop
    @BLACK
    0;JMP //otherwise do the black loop
	
(BLACK)
	@SCREEN
	D=A
	@address //begin coloring the screes black in SCREEN adress
	M=D
...
(INLOOP1)
    ...
    D=M
    @KBD
    D=D-A  // check if KBD-address=0 if so jump to loop
    @LOOP
    D;JGE
	...
	@address
	A=M
	M=-1 //set bits to -1 which is black
	
	@address
	M=M+1 //move adress to the next register in line
	
	@INLOOP1 //otherwise continue in-loop1
	0;JMP

(WHITE)
	@SCREEN
	D=A
	@address //begin coloring the screes white in SCREEN adress
	M=D
...
(INLOOP2)
	...
	@address
	A=M
	M=0 //set bits to 0 which is white
	
	@address
	M=M+1 //move adress to the next register in line
	
	@INLOOP2 //otherwise continue in-loop1
	0;JMP